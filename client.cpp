...
class client{
public:
 client(boost::asio::io_context &context, tcp::resolver::results_type& endpoints) : context(context), socket(context)
 {
     connect(endpoints);
 }
...
private:
 ...
 void readHeader(){
     boost::asio::async_read(socket,boost::asio::buffer(readMessage.data(),message::headerLength)
 ...
 void readBody(){
     boost::asio::async_read(socket,boost::asio::buffer(readMessage.body(),readMessage.bodyLength())
                             ,[this](boost::system::error_code ec,size_t){
         if (!ec){
 ...
 void write(){
     boost::asio::async_write(socket,boost::asio::buffer(writeMessage.front().data(),writeMessage.front().length())
                              ,[this](boost::system::error_code ec, size_t){
        if (!ec){
